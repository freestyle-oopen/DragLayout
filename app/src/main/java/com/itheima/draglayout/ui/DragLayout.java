package com.itheima.draglayout.ui;
...
public class DragLayout extends FrameLayout {
    ...
    public DragLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDragHelper = ViewDragHelper.create(this, 1.1f, callback);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        float x=ev.getX();
        if( status==Status.Draging ||x>mRange){
            return true;
        }
...
//            return child == mMainContent;
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            // 先考虑打开的情况
            if(xvel<2500 &&xvel>-2500 && mMainContent.getLeft() > mRange * 0.5f){
                // 速度比较小, 根据位置判断
                open();
            }else if(xvel<2500 &&xvel>-2500 && mMainContent.getLeft() <= mRange * 0.5f){
                close();
            } else if(xvel >= 2500){
                // 速度向右
                open();
    ...
    // 执行伴随动画
    @SuppressLint("NewApi") private void animViews(float percent) {
        mLeftMenu.setTranslationX(evaluate(percent, -mWidth * (5.0/6.0), 0f));
...
/*//        1. 左菜单: 缩放动画,
       ...
       mLeftMenu.setScaleX(0.5f + percent * 0.5f);
        ...
        ViewCompat.setScaleX(mLeftMenu, evaluate(percent, 1.0f, 1.0f));
        ViewCompat.setScaleY(mLeftMenu, evaluate(percent, 1.0f, 1.0f));

        // 平移动画 -mWidth * 0.5f -> 0
        ViewCompat.setTranslationX(mLeftMenu, evaluate(percent, -mWidth * (6.0/8.0), 0f));
        mLeftMenu.setTranslationX(evaluate(percent, -mWidth * (6.0/8.0), 0f));
        // 透明度动画 0.3f -> 1.0f
       ViewCompat.setAlpha(mLeftMenu, evaluate(percent, 1.0f, 1.0f));

//        2. 主界面: 缩放动画, 1.0 -> 0.8
       ViewCompat.setScaleX(mMainContent, evaluate(percent, 1.0f, 1.0f));
        ViewCompat.setScaleY(mMainContent, evaluate(percent, 1.0f, 1.0f));

//        3. 背景: 亮度变化, 黑色 -> 透明
        Drawable drawable = getBackground();
        drawable.setColorFilter((Integer)evaluateColor(percent, Color.BLACK, Color.TRANSPARENT),PorterDuff.Mode.SRC_OVER);*/
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        if(status==Status.Open){
            mMainContent.layout((int)(right*(5.0/6.0)), top, (int)(right*(11.0/6.0)), bottom);
            mLeftMenu.layout(0, 0, (int)(right*(5.0/6.0)), bottom);
        }
